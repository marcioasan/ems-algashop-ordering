...
public interface ShoppingCartPersistenceEntityRepository extends JpaRepository<ShoppingCartPersistenceEntity, UUID> {
    ...
    @Query("""
		...
		""")
    void updateItemPrice(@Param("productId") UUID productId, @Param("price") BigDecimal price); //9.7. Como lidar com atualizações em massa - 7', 10'10"
    ...
    @Query("""
		...
		WHERE
			i.productId = :productId
		""")
    void updateItemAvailability(@Param("productId") UUID productId, @Param("available") boolean available);//9.7. Como lidar com atualizações em massa - 12'30"
    ...
    @Query("""
		...
		""")
    void recalculateTotalsForCartsWithProduct(@Param("productId") UUID productId); //9.7. Como lidar com atualizações em massa - 13'10"
}
